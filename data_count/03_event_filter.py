        ...
        for bug_zip in zip_dir_list:
            ...
            for xml_file in xml_dir_list:
                ...
                for event in events_nodes:
                    ...
                    event_structure_king = event.attrib.get('StructureKind')
                    event_start_date = get_common_time(event.attrib.get('StartDate'))
                    event_structure_handle = event.attrib.get('StructureHandle')
                    # all_structureKind.add(event_structure_king)
                    if (event_kind == 'selection' or event_kind == 'edit') and event_structure_king == 'java':
                        event_sum += 1
                        write_xsl([bug, event_kind, event_structure_king, event_structure_handle, event_start_date])
